...
void INode::WriteI() {
	...
	if (0 == myUserCall.IOParam.count)
		return;
	// 使用临时变量遍历，防止base变成野指针
	unsigned char* userBuf = myUserCall.IOParam.base;
	int userCount = myUserCall.IOParam.count;
	int userOffset = myUserCall.IOParam.offset;
	while (UserCall::U_NOERROR == myUserCall.userErrorCode && userCount) 
	{
		lbn = userOffset / INode::BLOCK_SIZE;
		offset = userOffset % INode::BLOCK_SIZE;
		nbytes = Common::min(INode::BLOCK_SIZE - offset, userCount);
		if ((bn = this->Bmap(lbn)) == 0) {
			printf("[DEBUG] Bmap failed, lbn=%d, offset=%d, count=%d\n", lbn, offset, userCount);
		...
		// 新增调试和空指针检查
		if (!pCache) {
			printf("[ERROR] pCache is nullptr! bn=%d, lbn=%d, offset=%d, count=%d\n", bn, lbn, offset, userCount);
			myUserCall.userErrorCode = UserCall::U_ENOSPC;
			break;
		}
		if (!pCache->addr) {
			printf("[ERROR] pCache->addr is nullptr! pCache=%p, bn=%d\n", (void*)pCache, bn);
			myUserCall.userErrorCode = UserCall::U_ENOSPC;
			break;
		}
		if (!userBuf) {
			printf("[ERROR] IOParam.base is nullptr!\n");
			myUserCall.userErrorCode = UserCall::U_EBADF;
			break;
		}
		if (nbytes <= 0 || nbytes > INode::BLOCK_SIZE) {
			printf("[ERROR] nbytes invalid: %d\n", nbytes);
			myUserCall.userErrorCode = UserCall::U_EBADF;
			break;
		}
		printf("[DEBUG] WriteI: pCache=%p, addr=%p, base=%p, nbytes=%d, offset=%d, count=%d\n", (void*)pCache, (void*)pCache->addr, (void*)userBuf, nbytes, offset, userCount);

		//缓存中数据的起始写位置 写操作: 从用户目标区拷贝数据到缓冲区
		unsigned char* start = pCache->addr + offset;

		memcpy(start, userBuf, nbytes);
		userBuf += nbytes;
		userOffset += nbytes;
		userCount -= nbytes;
		...
		//普通文件长度增加
		if (this->i_size < userOffset)
			this->i_size = userOffset;
		this->i_flag |= INode::IUPD;
	}
	// 循环结束后再赋值回去
	myUserCall.IOParam.base = userBuf;
	myUserCall.IOParam.offset = userOffset;
	myUserCall.IOParam.count = userCount;
}
